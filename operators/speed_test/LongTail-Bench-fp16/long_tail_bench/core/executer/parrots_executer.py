...
from long_tail_bench.common import PatModes
...
class ParrotsExecuter(TorchAPIExecuter):
    ...
    def prepare(self, stage_mode):
        ...
        self._execute_func = (self._origin_func if not stage_args else pat(
            self._origin_func, **stage_args))
    ...
    def assert_tensor_eq(self, tensor_a, tensor_b, rtol, atol):
        ...
        if tensor_a.dtype != torch.float32:
            ...
            assert parrots.allclose(tensor_a,
                                    tensor_b,
                                    equal_nan=True,
                                    rtol=rtol,
                                    atol=atol)
    ...
    def save_timeline_start(self, case_name, stage_mode, saving_path):
        ...
        parrots.runtime.profile(enable=True,
                                file=self._timeline_saving_path,
                                use_scope=True)
