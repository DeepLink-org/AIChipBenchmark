...
def get_sample_config():
    return SampleConfig(
        ...
        url=
        "https://gitlab.bj.sensetime.com/parrots.fit/segmentbase2/-/blob/master/segmentbase2/models/losses/Edge_SmoothLoss.py#L21",  # noqa
        tags=[
            SampleTag.ViewAttribute, SampleTag.IfElseBranch, SampleTag.Reduce,
            SampleTag.ForLoop
        ])
...
def gen_np_args(N):
    ...
    logit = logit.astype(np.float16)
    edge_logit = np.random.randn(N, 1, 3, 3)
    edge_logit = edge_logit.astype(np.float16)
