...
from long_tail_bench.core.executer.xla_executer import XLAExecuter
...
def args_adaptor(np_args):
    rois = tf.convert_to_tensor(np_args[0]).cuda()
    deltas = tf.convert_to_tensor(np_args[1]).cuda()

    means = tf.convert_to_tensor(np_args[2]).cuda()
    stds = tf.convert_to_tensor(np_args[3]).cuda()
    max_shape = tf.convert_to_tensor(np_args[4]).cuda()

    return [rois, deltas, means, stds, max_shape]


def executer_creator():
    return XLAExecuter(delta2bbox, args_adaptor)
