...
import json
import math
from long_tail_bench.common import FrameType
from long_tail_bench.common.types import PatModes
...
def collect_func_and_perf_info(case_names, frame_types, settings,
                               frame_type_to_frame_modes):
    json_re = {}
    for frame in frame_types:
    ...
    for case_name in case_names:
        ...
        for frame in frame_types:
            for mode in frame_type_to_frame_modes[frame]:
                if mode.value is None:
                    continue
                if (frame.value in json_re
                        and case_name in json_re[frame.value] and mode.value
                        in json_re[frame.value][case_name]["times"]):
                    re.append(
            ...
            if frame.value in json_re and case_name in json_re[frame.value]:
                ...
                for k in json_re[frame.value][case_name]["errors"]:
                    error_info = (
                        error_info + str(k) + " Error:" +
                        json_re[frame.value][case_name]["errors"][k] + "\n")
                re.append(error_info)
            else:
                re.append(None)

        if (FrameType.Parrots.value in json_re
        ...
        if (FrameType.Parrots.value in json_re
                ...
                and json_re[FrameType.Parrots.value][case_name]["times"][
                    PatModes.S1.value] is not None
                and json_re[FrameType.Parrots.value][case_name]["times"][
                    PatModes.S5.value] is not None):
...
def save_to_csv(heads, case_names, all_results, result_dir):
    ...
    content_to_csv.append(
        ["Samples SpeedUp Geometric Mean:",
         cal_geometric_mean(s5_speedup)])
