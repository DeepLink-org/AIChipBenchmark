...
class BMNLoss(nn.Module):
    ...
    def pem_reg_loss(
        ...
        loss = (0.5 * torch.sum(loss * torch.ones_like(weights)) /
                torch.sum(weights))
...
def args_adaptor(np_args):
    pred_bm = torch.from_numpy(np_args[0]).cuda()
    pred_start = torch.from_numpy(np_args[1]).cuda()
    pred_end = torch.from_numpy(np_args[2]).cuda()
    gt_iou_map = torch.from_numpy(np_args[3]).cuda()
    gt_start = torch.from_numpy(np_args[4]).cuda()
    gt_end = torch.from_numpy(np_args[5]).cuda()
    bm_mask = torch.from_numpy(np_args[6]).cuda()
