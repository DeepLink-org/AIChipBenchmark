...
def configure_recipe(nodes: int = 1, gpus_per_node: int = 8):
    ...
    recipe = llm.qwen2_72b.pretrain_recipe(num_nodes=nodes, num_gpus_per_node=gpus_per_node,
    ...
    recipe.data=run.Config(
            ...
            paths="./qwen2_72b/arxiv_sample_text_document",
            seq_length=8192,
            micro_batch_size=data_bak.micro_batch_size,
            global_batch_size=data_bak.global_batch_size,
            tokenizer=run.Config(AutoTokenizer, "./model--qwen2--qwen2--72B/snapshots/999320f23a0d2d9883ccb110784a2bcf06da2305"),
            split='900,50,50',
            seed=2025,
    )
...
def run_pretraining():
    ...
    run.run(recipe, executor=executor, name="qwen2_72b_pretraining")
