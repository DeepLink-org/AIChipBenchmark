...
def get_requests(
    input_ids_path: str,
    output_length_path: str,
    num_requests: int,
    tokenizer: Tokenizer,
) -> List[Tuple[str, int, int]]:
    # Load the dataset.
    with open(input_ids_path, "r") as f1:
        lines_input_ids = f1.readlines()
    ids_list = []
    prompt_text_list = []
    for line in lines_input_ids:
        ids = line.strip().split(",")
        ids = [int(id_str) for id_str in ids]
        ids_list.append(ids)
        prompts = tokenizer.decode(ids)
        prompt_text_list.append(prompts)

    with open(output_length_path, "r") as f2:
        lines_output_length = f2.readlines()
    output_len_list = [int(line.strip()) for line in lines_output_length if line.strip()]

    assert len(ids_list) == len(prompt_text_list) == len(output_len_list), \
        f'Error. ids_list length is ({len(ids_list)}) \n' \
        f'       prompt_text_list length is ({len(prompt_text_list)})\n' \
        f'       output_len_list length is ({len(output_len_list)})'

    input_length = len(ids_list)
    tokenized_dataset = []
    for i in range(input_length):
        tokenized_dataset.append((prompt_text_list[i], ids_list[i], output_len_list[i]))
    ...
    for prompt, prompt_token_ids, output_len in tokenized_dataset:
        ...
        if prompt_len < 4 or output_len < 4:
            ...
            print(f"Prune too short sequences.")
            continue
        if prompt_len > 1024 or prompt_len + output_len > 2048:
            # Prune too long sequences.
            print(f"Prune too long sequences.")
            continue
        filtered_dataset.append((prompt, prompt_len, output_len))

    print(f"ids_list len is {input_length}")
    # Sample the requests.
    sampled_requests = filtered_dataset if input_length >= num_requests else random.choices(filtered_dataset, k=num_requests)
    print(f"sampled_requests[0] is {sampled_requests[0]}")
...
def parse_args():
    ...
    parser.add_argument('--input-ids',
                        type=str,
                        help='Where to get the prompts.',
                        default='./overall_input_token_ids.txt')
    parser.add_argument('--output-length',
                        type=str,
                        help='Where to get the output length.',
                        default='./overall_output_length.txt')
    # other args
...
def main():
    ...
    requests = get_requests(args.input_ids, args.output_length, args.num_prompts, engine.tokenizer)
