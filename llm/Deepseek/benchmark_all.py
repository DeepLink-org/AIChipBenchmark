...
log_prefix = "log"
# 要测试的并发数
NUM_CLIENTS = [8, 16, 1, 32, 64, 128]
# 要测试的输入和输出长度组合
input_lengths = [256, 512, 1024, 8192, 32768, 122880]
...
# 创建CSV文件
with open('test_results_deepseek_r1.csv', 'w', newline='') as csvfile:
    writer = csv.DictWriter(csvfile, fieldnames=csv_headers)
    writer.writeheader()

    for WORKER in NUM_CLIENTS:
        INPUT_NUM = 2 * WORKER
        # 遍历所有输入输出长度组合
        for input_len in input_lengths:
            for output_len in output_lengths:
                if input_len not in [256, 512] and output_len in [128, 512]:
                    continue
                print(f"\nTesting with num_clients={WORKER}, input_len={input_len}, output_len={output_len}")
                
                # 构建命令
                cmd = [
                    "python3", "/data/huangye/llm_profiler.py",
                    "--url", URL,
                ...
                log_file = f"{log_prefix}/test_log_bz{WORKER}_input{input_len}_output{output_len}.log"
                try:
                    # 执行命令并将输出写入日志文件
